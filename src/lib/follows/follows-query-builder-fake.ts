...
// defined a byte-identical copy): every filter method returns the same builder so a
// `.select().eq().in().order()` chain resolves however the code under test calls it, and the
// builder itself resolves (via `then`) to the given result — same shape
// `await supabase.from(...).select(...)` resolves to for real. Exposes all three filter methods
// (eq/in/order) so either test file's own chain shape is covered without needing its own copy.
