...
// rqtid=1 (club stats), rqtid=9 (countries), rqtid=10 (regions) and rqtid=11 (takeoffs) all
// render the identical "plainest response on the site" shape (see docs/flightlog-api.md): no
// page shell, no nav, no honeypot, no links at all — just `<table border=1>` with an unwrapped
// `<th>` header cheerio synthesizes into a leading `<tr>`, followed by zero or more
// `<tr><td>…</td></tr>` data rows. Confirmed empirically for rqtid=9 (zero <a> tags, zero
// hp-nav occurrences), for both rqtid=10 and rqtid=11 (Norway, country_id=160, and Bouvet
// Island, country_id=29 — same, in every capture of either endpoint), and for rqtid=1 (every
// cell is a plain Name/Flights/Distance/Time value, never a link, unlike the roster on the
// same club that DOES carry per-pilot links).
...
// Shared by parse-countries.ts, parse-regions.ts, parse-takeoffs.ts and parse-club-stats.ts.
// Returns every candidate data row — callers still have to run their own strict per-row
// extraction and floor check on top, the same division of labour parse-clubs.ts and
// parse-pilot-search.ts use.
